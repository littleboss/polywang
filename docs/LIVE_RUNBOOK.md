...
## 纸面结算对账（QUANT-20260914-01 / QUANT-20260915-01）
...
3. 未判定但已过 `event_end + PAPER_SETTLE_STUCK_GRACE_HOURS`（默认 48）：
   - UMA 为 Known/Finalized 且是完整集合 → 强制纸面结算，原因码 `PAPER_FORCE_SETTLE_UMA_FINALIZED`。
   - UMA disputed / 无唯一 winner 且是完整集合 → 默认 `PAPER_SETTLE_UNRESOLVED_POLICY=force_settle` 走终端 exception：兑付完整集合（`payout_per_share`），写 `UMA_DISPUTED` / `PAST_GRACE_NO_WINNER` 和原因码到 ledger / journal / `SETTLE_PAIR`，并记 `settlement_exception`。设 `stuck` 则只打 `SETTLEMENT_STUCK`、不关仓。
   - 未完成腿（unhedged）永不 force-settle，只打 `SETTLEMENT_STUCK`。
   `PAPER_SETTLE_STUCK_ALERT=1` 时对 stuck 打 CRITICAL 并写 `monitor-exceptions.jsonl`。exception 路径总会写一条 `settlement_exception`（若配置了 exception tape）。
4. 不绕过 Deterministic Policy Gate；实盘 redeem/merge 不变；费率地板不变。`ENABLE_NEGRISK_LIVE` / `POLYMARKET_LIVE_CONFIRM` 保持未设置。不要为了对账去中途拉停正在跑的 8039ea3 监督窗口。

```bash
PAPER_SETTLE_RECONCILE_INTERVAL_SEC=300
PAPER_SETTLE_STUCK_GRACE_HOURS=48
PAPER_SETTLE_STUCK_ALERT=0
PAPER_SETTLE_UNRESOLVED_POLICY=force_settle
```

`--health` 只读字段：`settlement_stuck`（过了 grace 仍未结的仓）、`settlement_exceptions`（已关仓的纸面终端 exception 数）和 `unhedged_leg_count`（完整集合应为 0）。
