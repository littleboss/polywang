...
纸面结算对账（QUANT-20260914-01）默认每 `PAPER_SETTLE_RECONCILE_INTERVAL_SEC`（300）秒向 Gamma 查询未结 `position_id` / NegRisk 篮子。市场已判定时复用现有 `market_resolved` → `SETTLE_PAIR` / `settle_baskets`（NR `SETTLED`），按 resolution 兑付并释放 `capital_reserved`，现金和暴露应在 $0.01 内闭合。超过 `event_end + PAPER_SETTLE_STUCK_GRACE_HOURS`（48）仍未判定时：UMA Known/Finalized 且是完整集合则强制纸面结算，否则打上 `SETTLEMENT_STUCK`；`PAPER_SETTLE_STUCK_ALERT=1` 时写告警。实盘 redeem/merge、费率地板和 Policy Gate 都不动；不会中途改正在跑的监督窗口。
