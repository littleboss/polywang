...
纸面结算对账（QUANT-20260914-01 / QUANT-20260915-01）默认每 `PAPER_SETTLE_RECONCILE_INTERVAL_SEC`（300）秒向 Gamma 查询未结 `position_id` / NegRisk 篮子。市场已判定时复用现有 `market_resolved` → `SETTLE_PAIR` / `settle_baskets`（NR `SETTLED`），按 resolution 兑付并释放 `capital_reserved`，现金和暴露应在 $0.01 内闭合。超过 `event_end + PAPER_SETTLE_STUCK_GRACE_HOURS`（48）仍未判定时：UMA Known/Finalized 且是完整集合则强制纸面结算；UMA disputed / 无唯一 winner 时，`PAPER_SETTLE_UNRESOLVED_POLICY=force_settle`（默认）走带原因码的纸面终端 force-settle（`PAPER_FORCE_SETTLE_UMA_DISPUTED` / `PAPER_FORCE_SETTLE_PAST_GRACE_NO_WINNER`，账本/journal/`SETTLE_PAIR` 都写审计），未完成腿仍打 `SETTLEMENT_STUCK`。设 `stuck` 可回到只告警、不关仓。`PAPER_SETTLE_STUCK_ALERT=1` 时对 stuck 写 CRITICAL。实盘 redeem/merge、费率地板和 Policy Gate 都不动；不会中途改正在跑的监督窗口。
...
它只研究 Yes 和 No 两腿同时买入后合计支付 1.00 的二元市场。选市会先拉一个按成交量过滤的候选池（`MARKET_SCAN_POOL`），再按 **最低 yes_ask+no_ask** 排序（Gamma `outcomePrices` 作抓取代理，有盘口后用 live touch）。Gamma 若返回同一 `market_id` / `condition_id` 的重复二元行，会在 keep slice 前去重（保留排序更靠前的那条，并记 `dropped_duplicate_market_id`），这样 `MARKET_LIMIT=200` 不会把重复 id 送进 runner；跨不同 `market_id` 的 token / condition 碰撞仍会 fail-close。总和相同时优先 geopolitics（费率 0）。不再把 longshot 的 1-tick 理论（`ticks_to_breakeven` / `one_tick_net`）当作主排序。纸面 NegRisk 独立 n 腿路径默认打开（`ENABLE_NEGRISK_EXECUTION` 未设置即为开，`ENABLE_NEGRISK_LIVE` 保持未设置），账本写入 `paper-negrisk.json`，不会写 `live-orders.json`。下一窗宇宙默认 `MARKET_LIMIT=200`、`NEGRISK_MARKET_LIMIT=40`，只在启动时读 CLI `--markets` / 环境变量，**不会**中途改正在跑的监督窗口。纸面 NegRisk 在 execute 前硬限制未结篮子数（`PAPER_MAX_OPEN_NEGRISK`，默认 8）和已预留资金（`PAPER_MAX_NEGRISK_RESERVED_USD`，默认 `0.40 * initial_cash`，例如 $1000 本金对应 $400）；超限记入 SCAN `risk_skip_open_negrisk` / `risk_skip_negrisk_capital`，现金不足仍走 `risk_skip_cash`。纸面 admit 默认只做 2 腿、腿价在 `PAPER_NEGRISK_MIN_LEG_PRICE`/`PAPER_NEGRISK_MAX_LEG_PRICE`（0.05–0.95）之间的 `BUY_ALL_YES`（`PAPER_NEGRISK_MAX_LEGS=2`，`PAPER_NEGRISK_ALLOW_BUY_ALL_NO=0`）；多腿记 `negrisk_too_many_legs`，极端价记 `negrisk_extreme_price`，`BUY_ALL_NO` 记 `negrisk_direction_disabled`。观察日志仍可留下被过滤的形态。选宇宙时优先 `outcomes_count==2`（腿数最少）再填满 `NEGRISK_MARKET_LIMIT`。机会必须使用订单簿中的实际 ask 和深度，并覆盖两腿 taker 费用、可选 merge gas（`MERGE_GAS_USD`）、资金安全缓冲和最大仓位；纸面成交写入 `paper-ledger.json`，程序重启后会恢复账本。两腿仍是顺序 FOK，不是原子交易，`is_risk_free` 恒为 false。纸面单笔上限默认按账本现金动态计算（QUANT-20260915-02）：`effective_max = clamp(cash * MAX_ORDER_FRACTION_OF_CASH, MAX_ORDER_FLOOR_USD, MAX_ORDER_CAP_USD)`，默认 `0.025 / $5 / $100`（例如现金 $1000 → $25，现金 $10000 触顶 $100）。`FRACTION>0` 开启动态（默认）；设 `MAX_ORDER_FRACTION_OF_CASH=0` 且显式 `MAX_ORDER_USD>0`（或 `--max-order`）则回到固定上限。二元和 NegRisk 共用同一 helper，每次 admit/execute 前按当时账本现金（优先 `cash_after_reserved`，否则 `cash`）重算；SCAN / OPEN 日志带 `effective_max_order_usd`。小额阶段保持 `AUTO_MERGE_COMPLETE_SETS=0`：merge gas 是固定成本，未实测就填 0 并打开自动 merge 会把亏损单显示成盈利。费率地板不变：`post_fee_net >= min_net_profit`（0.05）、`min_return=0.002`、`safety_buffer=0.02`。
...
实盘还会从 `live-risk.json` 恢复风险状态。默认总暴露上限为账户权益的 25%，单市场上限为 5%，最多 10 个未结 pair；可分别用 `LIVE_MAX_TOTAL_EXPOSURE_FRACTION`、`LIVE_MAX_MARKET_EXPOSURE_FRACTION`、`LIVE_MAX_OPEN_PAIRS` 和 `LIVE_RISK_EQUITY_USD` 配置。User Stream 负责实时成交回报，REST 对账默认每 15 秒检查已知订单、增量成交和余额，并每 `LIVE_FULL_RECOVERY_CYCLES` 轮（默认 20 轮）做一次 open-order 孤儿恢复；启动时仍会做完整恢复。创建 `live-kill-switch` 文件，或设置 `POLYMARKET_KILL_SWITCH=1`，会持久化停止新单、撤销所有未完成订单，并对方向性单腿库存发 FAK SELL；已匹配的 Yes+No 组合库存不会自动市价甩卖。清除文件不会自动解除已经持久化的 halt，必须人工检查 `live-risk.json` 后再决定是否恢复。已提交但等待超时的 merge/redeem 交易会在后续对账中查询其原 transaction ID/hash，确认成功后自动完成账本，绝不会重新提交。市场扫描还会拒绝 Yes/No 两腿时间戳差超过 `MAX_LEG_SKEW_MS`（默认 1000）的盘口。
