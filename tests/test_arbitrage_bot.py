...
class ScanRejectAndGammaTests(unittest.TestCase):
    ...
    def test_scan_reject_counter_flush_sums_to_attempts_and_logs_touch(self):
        ...
        self.assertIn("risk_skip_open_negrisk=0", lines[0])
        self.assertIn("risk_skip_negrisk_capital=0", lines[0])
    ...
    def test_scan_reject_counter_risk_skip_reasons_do_not_double_count(self):
        ...
        self.assertEqual(
            classify_risk_skip(ValueError("maximum number of open NegRisk baskets reached")),
            "open_negrisk",
        )
        self.assertEqual(classify_risk_skip(ValueError("NegRisk reserved capital limit")), "negrisk_capital")
        counter = ScanRejectCounter(flush_interval_s=3600.0, logger=Capture())
        ...
        counter.record("risk_skip", detail="open_negrisk")
        counter.record("risk_skip", detail="negrisk_capital")
        self.assertEqual(counter.counts["risk_skip"], 5)
        self.assertEqual(counter.attempts, 5)
        self.assertEqual(counter.risk_skip_reasons["cash"], 1)
        self.assertEqual(counter.risk_skip_reasons["open_negrisk"], 1)
        self.assertEqual(counter.risk_skip_reasons["negrisk_capital"], 1)
        counter.flush()
        self.assertEqual(len(lines), 1)
        self.assertIn("risk_skip=5", lines[0])
        ...
        self.assertIn("risk_skip_open_negrisk=1", lines[0])
    ...
    def test_scan_rejects_flush_includes_risk_skip_reason_and_keeps_floors(self):
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            ...
            self.assertIn("risk_skip_open_negrisk=", line)
            self.assertIn("risk_skip_negrisk_capital=", line)
